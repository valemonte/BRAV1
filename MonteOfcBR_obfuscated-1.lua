--[[ Monte HUB | Minimal (comments stripped, no rename — stable for loader) ]]
...
local stealMovementMethod    = "Deslize Suave"
...
local function TravelToDestination(targetPos, speed, isApproach)
    if stealMovementMethod == "Deslize Voando" then
        return TravelFlyDirect(targetPos, speed, isApproach)
    elseif stealMovementMethod == "Caminhada Segura" then
...
local function isMutationAllowed(muts, record, filter)
    local isParasite = (record and record.HasParasite == true)
        or (type(muts) == "table" and (table.find(muts, "Parasite") or table.find(muts, "Monstruoso")))
        or (record and (record.BaseMutation == "Parasite" or record.BaseMutation == "Monstruoso"))
    if not filter or type(filter) ~= "table" then return true end
    local count = 0
    for _ in pairs(filter) do count = count + 1 end
    if count == 0 then return true end
    ...
    for _, opt in pairs(filter) do
        if type(opt) == "string" then
            if opt == "Apenas Normal" and not hasMut and not isParasite then
                allowed = true
            elseif opt == "Apenas Mutado" and (hasMut or isParasite) then
                allowed = true
            elseif (opt == "Parasita / Infestado" or opt == "Monstruoso") and isParasite then
                allowed = true
            elseif opt == "Apenas Silver" and type(muts) == "table" and table.find(muts, "Silver") then
                allowed = true
            elseif opt == "Apenas Gold" and type(muts) == "table" and (table.find(muts, "Gold") or table.find(muts, "Golden")) then
                allowed = true
            elseif opt == "Apenas Rainbow" and type(muts) == "table" and table.find(muts, "Rainbow") then
                allowed = true
            end
        end
    end
        ...
        if record.State == "Slot" and record.BoundsCFrame then
            ...
            if not isIgnored and (not stealBigEggsOnly or isBigEgg(record)) then
                ...
                if areaOk and rarityOk and mutOk then
                    ...
                    if record.HasParasite == true or (type(muts) == "table" and (table.find(muts, "Parasite") or table.find(muts, "Monstruoso"))) then
...
function Boss.FindTarget()
    ...
    if best and Boss.arenaApproach == "Cristais Primeiro" then
        return best, "Crystal"
    end
...
local EggsTab     = Window:AddTab({ Name = "Ovos", Subtitle = "Roubar, chocar e plantar", Icon = "crown" })
local BaseTab     = Window:AddTab({ Name = "Base", Subtitle = "Fazenda e treino", Icon = "bolt" })
local CombatTab   = Window:AddTab({ Name = "Combate", Subtitle = "Bastão, tapas e defesa", Icon = "combat" })
local PlayerTab   = Window:AddTab({ Name = "Jogador", Subtitle = "Movimento e teleporte", Icon = "player" })
local SettingsTab = Window:AddTab({ Name = "Configurações", Subtitle = "Configs e descarregar", Icon = "gear" })
local StealSub = EggsTab:AddSubTab("Roubo Automático")
local HatchSub = EggsTab:AddSubTab("Chocar e Plantar")
local EggEspSub = EggsTab:AddSubTab("ESP de Ovos")
StealSub:AddToggle({
...
StealSub:AddMultiDropdown({
    Name = "Filtrar por Raridade (Multi-seleção)", Options = RARITY_NAMES, Default = {}, Flag = "steal_rarities",
    Callback = function(selectedList) selectedStealRarities = selectedList end
})
StealSub:AddMultiDropdown({
    Name = "Filtrar por Área (Multi-seleção)", Options = AREA_NAMES, Default = {}, Flag = "steal_areas",
    Callback = function(selectedList) selectedStealAreas = selectedList end
})
StealSub:AddMultiDropdown({
    Name = "Filtrar por Mutação (Multi-seleção)", Options = MUTATION_FILTERS, Default = {}, Flag = "steal_muts",
...
HatchSub:AddButton({
    ...
    Callback = safeCallback(function()
        ...
        Notify("Chocar", "Chocou " .. count .. " ovo(s)", "Success")
    end)
})
HatchSub:AddButton({
    ...
    Callback = safeCallback(function()
        ...
        Notify("Plantar Ovos", "Plantou " .. count .. " ovo(s) no curral", "Success")
    end)
})
EggEspSub:AddToggle({
...
do
local UpgradesSub = BaseTab:AddSubTab("Fazenda e Esteira")
local PetsSub     = BaseTab:AddSubTab("Pets e Bolsa")
local SalesSub    = BaseTab:AddSubTab("Venda Automática")
local EventsSub   = BaseTab:AddSubTab("Eventos e Chefes")
local RewardsSub  = BaseTab:AddSubTab("Resgatar Recompensas")
...
SalesSub:AddMultiDropdown({
    Name = "Filtrar Raridades de Pets para Venda", Options = RARITY_NAMES, Default = {}, Flag = "sell_pet_rarities",
...
SalesSub:AddMultiDropdown({
    Name = "Filtrar Raridades de Ovos para Venda", Options = RARITY_NAMES, Default = {}, Flag = "sell_egg_rarities",
...
EventsSub:AddButton({
    ...
    Callback = safeCallback(function()
        ...
        if snap and snap.Open then
            ...
            Notify("Arena do Chefe", "ABERTA - " .. tostring(math.floor(hp)) .. "/" .. tostring(math.floor(maxHp)) .. " HP", "Success")
        else
...
do
local BatSub   = CombatTab:AddSubTab("Aura de Bastão")
local GuardSub = CombatTab:AddSubTab("Defesa e Guardas")
BatSub:AddToggle({
...
do
local MoveSub     = PlayerTab:AddSubTab("Movimento")
local AreaTpSub   = PlayerTab:AddSubTab("Viajar para Área")
local PlotTpSub   = PlayerTab:AddSubTab("Viajar para Plot")
local PlayerTpSub = PlayerTab:AddSubTab("Viajar até Jogador")
local PerfSub     = PlayerTab:AddSubTab("Visuais e Desempenho")
MoveSub:AddToggle({
...
AreaTpSub:AddDropdown({
    Name = "Selecionar Área", Options = areaKeys, Items = areaKeys, Default = "Base / Plot", Flag = "tele_area",
...
AreaTpSub:AddButton({
    ...
    Callback = safeCallback(function()
        ...
        if pos then
            Notify("Viagem", "Viajando para " .. selectedAreaTp, "Info")
            TravelRoadPath(pos, glideSpeed or 200)
            Notify("Viagem", "Chegou em " .. selectedAreaTp, "Success")
        else
...
local plotOptions = { "Plot 1", "Plot 2", "Plot 3", "Plot 4", "Plot 5", "Plot 6", "Plot 7", "Meu Plot" }
PlotTpSub:AddDropdown({
    Name = "Selecionar Plot", Options = plotOptions, Items = plotOptions, Default = "Meu Plot", Flag = "tele_plot",
    Callback = function(v) selectedPlotNum = v end
})
PlotTpSub:AddButton({
    Name = "Viajar para o Plot", Primary = true,
    Callback = safeCallback(function()
        local slotNum = selectedPlotNum == "Meu Plot" and GetLocalSlot() or tonumber(selectedPlotNum:match("%d+")) or 1
        ...
        if targetPos then
            ...
            Notify("Plot", "Chegou no Plot " .. tostring(slotNum), "Success")
        else
...
local function GetPlayerList()
    ...
    table.sort(names)
    if #names == 0 then names = { "(nenhum outro jogador)" } end
    return names
end
local playerDropdown = PlayerTpSub:AddDropdown({
    Name = "Selecionar Jogador", Options = GetPlayerList(), Items = GetPlayerList(), Default = nil, Flag = "tele_plr",
...
PlayerTpSub:AddButton({
    ...
    Callback = safeCallback(function()
        ...
        if tHrp then
            ...
            Notify("Jogador", "Chegou em " .. selectedPlayerName, "Success")
        else
...
PerfSub:AddButton({
    ...
    Callback = safeCallback(function()
        ...
        Notify("Desempenho", "Removeu " .. count .. " modelo(s) de pet", "Success")
    end)
})
end
do
local ConfigSub = SettingsTab:AddSubTab("Configuração")
if HAS_CONFIG then
    ConfigSub:AddInput({
        Name = "Nome da Config", Default = CONFIG_NAME, Flag = "cfg_name",
    ...
    ConfigSub:AddButton({
        ...
        Callback = safeCallback(function()
            ...
            Notify("Config", ok and ("Config '" .. CONFIG_NAME .. "' salva") or ("Falha ao salvar: " .. tostring(err)), ok and "Success" or "Error")
        end)
    })
    ConfigSub:AddButton({
        ...
        Callback = safeCallback(function()
            ...
            if ok then
                ResyncAll()
                Notify("Config", "Config '" .. CONFIG_NAME .. "' carregada", "Success")
            else
